	...
	partial class MainWindow {
        ...
        **-----------------------------------------------------------------------------------------------------------*/
		private bool SaveDirtyDocumentOnNew() {
			...
			if (m_bIsDirtyDocument) {
				...
				// Show message box.
				MessageBoxResult mrSaveFile = MessageBox.Show(this, strMessageBoxText, Properties.Resources.MessageBox_SaveDirtyDocumentOnNew_Caption, MessageBoxButton.YesNoCancel, MessageBoxImage.Warning);
				switch (mrSaveFile) {
        ...
        **-----------------------------------------------------------------------------------------------------------*/
		private bool SaveDirtyDocumentOnOpen() {
			...
			if (m_bIsDirtyDocument) {
				...
				// Show message box.
				MessageBoxResult mrSaveFile = MessageBox.Show(this, strMessageBoxTExt, Properties.Resources.MessageBox_SaveDirtyDocumentOnOpen_Caption, MessageBoxButton.YesNoCancel, MessageBoxImage.Warning);
        ...
        **-----------------------------------------------------------------------------------------------------------*/
		private void UpdateRecentFileList() {
			m_miFile_RecentFiles.Items.Clear();
			...
			for (int i = 0; i < m_colRecentFiles.Count && i < m_iMaxRecentFiles; i++) {
				m_miFile_RecentFiles.Items.Add(CreateRecentFileItem(i));
			}
		}
        ...
        **-----------------------------------------------------------------------------------------------------------*/
		private MenuItem CreateRecentFileItem(int index) {
			// Create an item
			MenuItem tmiRecentFile = new MenuItem();
		...
		private bool Save(string savePath) {
			...
			if (string.IsNullOrEmpty(savePath)) {
				...
				if (m_txTextControl.Save(StreamType.All, svsSaveSettings,
					...
					SaveSettings.DialogSettings.SaveSelection) == WPF.DialogResult.OK) {
					return false;
				}
			}
			else {
        ...
        **-----------------------------------------------------------------------------------------------------------*/
		private void SignOut() {
			...
			m_miFile_SignIn.IsEnabled = true;
			m_miFile_SignIn.Visibility = Visibility.Visible;

			// Hide the [Current User] item.
			m_miFile_CurrentUser.IsEnabled = false;
			m_miFile_CurrentUser.Visibility = Visibility.Collapsed;
		}
        ...
        **-----------------------------------------------------------------------------------------------------------*/
		private bool SaveDirtyDocumentOnExit() {
			...
			if (m_bIsDirtyDocument) {
				...
				MessageBoxResult mrSaveFile = MessageBox.Show(this, strMessageBoxTExt, Properties.Resources.MessageBox_SaveDirtyDocumentOnExit_Caption, MessageBoxButton.YesNoCancel, MessageBoxImage.Warning);
				switch (mrSaveFile) {
					case MessageBoxResult.Yes:
						bKeepGoing = Save(m_strActiveDocumentPath);
						break;
